...
class ShaderStringAsset extends Asset {
  ...
  generate() {
    // If production mode is active, then we compress the shaders
    if (process.env.NODE_ENV === 'production' || Boolean(process.env.SHADER_FORCE_COMPRESS)) {
      const noComments = removeComments(this.code);
      const noSpaces = removeSpace(noComments);

      const compressed = JSON.stringify(noSpaces)
      .replace(/\u2028/g, '\\u2028')
      .replace(/\u2029/g, '\\u2029');

      return { 'js': `module.exports = ${JSON.stringify(compressed)}` };
    }

    // Otherwise, we simply copy the shaders
    else {
...
module.exports = ShaderStringAsset;
